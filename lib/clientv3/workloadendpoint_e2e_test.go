...
var _ = testutils.E2eDatastoreDescribe("WorkloadEndpoint tests", testutils.DatastoreEtcdV3, func(config apiconfig.CalicoAPIConfig) {
	...
	spec1_1 := apiv3.WorkloadEndpointSpec{
		Node:          "node-1",
		Orchestrator:  "k8s",
		Pod:           "abcdef",
		ContainerID:   "a12345a",
	...
	spec1_2 := apiv3.WorkloadEndpointSpec{
		Node:          "node-1",
		Orchestrator:  "k8s",
		Pod:           "abcdef",
		ContainerID:   "a12345a",
	...
	spec2_1 := apiv3.WorkloadEndpointSpec{
		Node:          "node-2",
		Orchestrator:  "cni",
		Endpoint:      "eth0",
	...
	spec2_2 := apiv3.WorkloadEndpointSpec{
		Node:          "node-2",
		Orchestrator:  "cni",
		Endpoint:      "eth0",
		ContainerID:   "a232323a",
	...
	DescribeTable("WorkloadEndpoint e2e CRUD tests",
		func(namespace1, namespace2, name1, name2 string, spec1_1, spec1_2, spec2_1 apiv3.WorkloadEndpointSpec) {
			...
			_, outError := c.WorkloadEndpoints().Update(ctx, &apiv3.WorkloadEndpoint{
			...
			res2, outError := c.WorkloadEndpoints().Create(ctx, &apiv3.WorkloadEndpoint{
			...
			res, outError = c.WorkloadEndpoints().Update(ctx, &apiv3.WorkloadEndpoint{
	...
	Describe("WorkloadEndpoint watch functionality", func() {
		It("should handle watch events for different resource versions and event types", func() {
			...
			testWatcher1.ExpectEvents(apiv3.KindWorkloadEndpoint, []watch.Event{
				{
					Type:   watch.Added,
					Object: outRes2,
				},
			...
			Expect(err).NotTo(HaveOccurred())
			testWatcher2.ExpectEvents(apiv3.KindWorkloadEndpoint, []watch.Event{
				{
					Type:   watch.Added,
					Object: outRes1,
				},
			...
			if config.Spec.DatastoreType == apiconfig.EtcdV3 {
				...
				testWatcher2_1.ExpectEvents(apiv3.KindWorkloadEndpoint, []watch.Event{
					{
						Type:   watch.Added,
						Object: outRes1,
					},
			...
			testWatcher3.ExpectEvents(apiv3.KindWorkloadEndpoint, []watch.Event{
				{
					Type:   watch.Added,
					Object: outRes3,
				},
			...
			testWatcher4.ExpectEvents(apiv3.KindWorkloadEndpoint, []watch.Event{
				{
					Type:   watch.Added,
					Object: outRes1,
				},
